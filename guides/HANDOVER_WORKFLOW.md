# Developer → AIFactory Handover Workflow

> "I'm 20 minutes into a Claude Code session. The refactor is bigger than I thought. I type `/handover`, Claude summarises what we've been discussing, hands it to AIFactory's autonomous pipeline, and gives me a URL. I go to lunch. I come back to a draft PR + a QA report waiting for my review."

This guide covers the **`/handover` slash command** — the 1-keystroke way to move a task from a live Claude Code session into AIFactory's autonomous build pipeline.

## Why this exists

You have two complementary tools:

| | Claude Code (interactive) | AIFactory (autonomous) |
|---|---|---|
...
1. **You're in a Claude Code session in a repo with the AIFactory `.mcp.json`**.
   This file registers the stdio MCP server. The AIFactory monorepo already has one at the root.

2. **The AIFactory web-server is running**.

   ```bash
   cd apps/web-server && python -m server.main
   ```

   It listens on `http://localhost:3101` by default. Override with `AIFACTORY_API_URL`.

3. **The API token exists at `~/.aifactory/.token`** (the web-server creates this on first start and prints the value once). Override path with `AIFACTORY_API_TOKEN_FILE`.

That's it. The MCP server (`apps/backend/mcp_server/aifactory_server.py`) is started by Claude Code on-demand when you open the repo, with the right env vars set in `.mcp.json`.
...
You: /handover
Claude: ✅ Handed off to AIFactory.

         • Task: spec-099-jwt-refresh-tokens
         • Title: Add JWT refresh tokens to auth middleware
         • Track at: http://localhost:3101/tasks/p1:spec-099-jwt-refresh-tokens

         AIFactory's planner runs first — when it hits the plan-review gate
         the portal will show the implementation plan for your approval.
```

Claude reads the recent conversation, summarises it, picks a title, and calls `mcp__aifactory__task_create_and_run` under the hood.
...
Claude: I'd want to make sure I capture the right task — we've only had 2
        ...
        what you want AIFactory to build? Or paste the relevant requirements?
```
...
1. **Status: `created` → `planning`** — AIFactory's planner agent gathers context, writes `spec.md`, produces `implementation_plan.json`.

2. **Status: `planning` → `human_review`** — the plan is ready for your approval. The portal shows it; you can also poll from Claude Code with `mcp__aifactory__task_status` or open the URL directly.

3. **You approve (or reject) the plan** — via the portal, OR from Claude Code with `mcp__aifactory__task_approve_plan` / the M2 `mcp__aifactory__task_create_and_run`-style flow.
...
5. **A draft PR appears** when the build completes — open it from the portal or run `mcp__aifactory__task_create_pr` if you want to drive that from Claude Code too.
...
| `Error: AIFactory web-server not reachable...` | Start the web-server: `cd apps/web-server && python -m server.main` |
| `Error: AIFactory API token not found...` | The web-server prints the token on first start; regenerate via the web UI if you lost it |
| `Error: ... rejected — regenerate via the web UI` | The token at `~/.aifactory/.token` doesn't match what the DB has — regenerate |
| Claude picks the wrong AIFactory project | Manually run `mcp__aifactory__project_list`, then re-run `/handover` with the right project context |
...
- `mcp__aifactory__task_create_and_run` — the primitive `/handover` wraps
- `mcp__aifactory__task_status` — poll a running task
- `mcp__aifactory__task_approve_plan` — approve from the chat
- `mcp__aifactory__task_get_logs` — read agent output
...
This V1 deliberately scopes to **Claude Code users in the AIFactory repo**. The MCP control-plane Epic (#50) shipped tonight built the foundation; this V1 is the polish on top.

**V2 follow-ups (planned)**:

- **Remote MCP `aifactory.create_and_run` parity** — so Cursor / Continue.dev users can hand off from their IDE. The Remote MCP server (`/api/mcp-remote/sse`) currently lacks the create+run tool; the V1.1 PR deliberately deferred it.
- **`aifactory handover` CLI binary** — for IDEs without MCP support (JetBrains, plain terminal).
...
AIFactory's V1 differentiates by being **self-hosted, customer-controlled, audit-traceable, and reusing the same agent pipeline interactive users already trust** — there's no new "background-mode" variant, the same planner → coder → QA → human-review runs on every task whether it was started via the portal or `/handover`.
