...
tags: [ai-coding, self-hosted, compliance, aifactory]
---
...
I kept hearing versions of this. And the more I looked, the more I realized it isn't a niche
complaint — it's the unspoken default for a huge slice of the industry. So I built something for it,
open-sourced it, and this post is about why.

## The problem isn't capability. It's trust.

We're past the point of arguing whether AI can write code. It can. The models are good enough that
the interesting question has moved: *can you trust what it produced, and can you prove where it came
from?*
...
The industry shipped autonomy and skipped accountability. For a solo dev hacking on a side project,
fine. For a regulated team, that's a wall.
...
- **Opaque actions.** Most agents give you a diff, not a defensible record of *what they did, in
  what order, and what you approved.*
- **Lock-in.** Betting your dev workflow on one vendor's model and pricing is its own risk.
...
That's AIFactory. It turns a task into shipping code through a pipeline you watch and verify:
**spec → plan → code → QA**, with human-review gates at each step. You bring your own model —
Claude, OpenAI, Gemini, Codex, or a local Ollama / OpenAI-compatible endpoint — and you own the
infrastructure it runs on.

*[Here I'll walk through one real task end-to-end: the spec it wrote, the plan I approved, the diff
it produced in an isolated worktree, the QA report, and — the part that matters for this whole
argument — the audit log entry that records exactly what happened. Screenshots / short clip.]*

It's open source (MIT) and I'm building it solo, full-time. There's a separate enterprise edition
for organizations that need multi-tenant isolation, SAML/SCIM, and signed audit evidence — that's
what funds the open core — but the core pipeline is free and is the part most people need.
