...
- **Test evidence is scoped to the subtask being completed, so a verification subtask can no longer ride an earlier one's test run (#1187).** The #851 honesty gate asks "did a real test command run", and read `read_test_evidence(spec_dir)` — a summary of every run recorded for the whole *build*. So the second and every later verification subtask inherited the first one's green `pytest -q` and passed having executed nothing. Reproduced on `origin/dev`: with an earlier subtask's `pytest -q` on record, a subtask whose only declared deliverable was `docs/plans/030-testing-strategy.md` completed green, refusal `None`. The gate now asks "did a test command run *while this subtask was being worked*". The window boundary is written by the harness, in the same append-only ledger, by the one funnel that accepts a completion: `record_subtask_completed` appends a marker when a subtask is allowed to complete, and everything after the last marker naming a *different* subtask is this subtask's own work. Not the plan's `started_at` — nothing writes it (`Subtask.start()` has no caller in either engine), so that window would fall back to build-wide on every real build, and it would be model-supplied, which is the one thing a gate must not depend on. Kept off honest work: only verification subtasks are asked the question at all, so a subtask that correctly runs no tests is untouched; a marker for the *same* subtask is not a boundary, so re-reporting a completion is not refused for the run it already made; and the marker is written only after the gate accepts, so a refusal leaves the runs there for the retry it asks for. Both engines now mean the same thing by it — the wave path was already per-subtask (a child records into its own worktree spec dir, #1178) while the serial path was build-wide, so "a test ran" depended on which path a build took. Backward compatible: every ledger line on disk today is a run with no marker, so an existing evidence file reads exactly as it did and nothing already completed is retroactively refused. Shares the single escape hatch `AIFACTORY_TEST_EVIDENCE_GATE=off`; no second flag. Coder-path code that runs inside the build Job, so it ships via the `-nix` build image.

- **The PR that Approve opens is mergeable again: current base, and no factory-internal file in the diff (#1106).** Found on camera while recording the Factory#245 HITL governance demo — Approve opened a PR and GitHub refused the merge with `mergeable: false, mergeable_state: dirty`, conflicting on `.aifactory-status`, a factory-internal file with no business being in the PR. Two defects compounded, and either alone breaks the platform's human-oversight control. **Stale base:** the project checkout a build runs in is cloned once and reused, so its *local* base branch drifts behind `origin/<base>` by however many merges have landed (observed: local `main` one merge and a full day behind), and `WorktreeManager.create_worktree` cut the task branch from that stale ref — so the PR was based on an old commit and raced the next merge. Only the kubejob backend refreshed, pre-dispatch (#960); the subprocess backend never did. `create_worktree` now fetches `origin/<base>` and cuts from the fetched tip, which fixes both backends at the one point they route through. It deliberately does not move the local ref: the #960 `reset --hard` is safe only on the disposable kubejob clone, not on the shared checkout the subprocess backend runs concurrent builds and merge-backs against. **A fetch that fails now raises**, rather than falling back to the local ref — falling back *is* the bug, producing a green-looking build whose PR cannot merge, discovered only when a human clicks Approve. There is deliberately no "`origin/<base>` matches, so it must be fine" escape hatch: a remote-tracking ref is the memory of an earlier fetch, not evidence of currency, and in a checkout that never fetched it equals the stale local branch exactly. Only a repo with no `origin` at all proceeds, having nothing to be stale against; `gh auth setup-git` runs first so a missing credential helper is not mistaken for a broken remote, and `GIT_TERMINAL_PROMPT=0` plus a timeout keep the new network call from hanging a headless build instead of failing it. **Committed bookkeeping:** `.aifactory-status` (the ccstatusline file) was written at the *repo root* of the worktree and rewritten on every subtask, so in any project that already tracks it the coder staged it straight into the PR; the pathspec-exclude meant to prevent that failed outright in the pod, taking the whole safety-net `git add` down with it, and a `.gitignore` entry does nothing for an already-tracked file. `StatusManager` now writes `.aifactory/status.json` inside the gitignored runtime dir, so no repo-root file exists to stage, and the safety-net commit stages then unstages the bookkeeping instead of excluding it by pathspec — a pathspec quirk can no longer take the whole add down, and its failure logs at ERROR instead of being swallowed as a warning. Worktree commit/diff stats now compare against the remote base to match what the branch was cut from, so a two-commit task does not report dozens in the cockpit.

- **The merged tree is now verified against the service's real entrypoint, once, at the end of every build (#1123).** #1111 closed the per-subtask hole, but it judges the worktree a subtask is working in, which in a parallel wave is not the merged tree — so a worker that breaks a sibling's route when it merges later, and wiring that can only exist after the merge (the planner rightly splits "write the module" from "register it"), both stayed invisible. Measured on the final merged `aifactory/101-vat-quote-endpoint-with-half-u` tree with its own 91 tests: the suite passes **91/91 both with and without** `app.include_router(vat_quote_router)`, because every test naming `/api/quote` builds a throwaway `FastAPI()`. Deleting the one line that makes the endpoint reachable changed nothing; the feature shipped with zero coverage of the endpoint a user can call. The trailing-gate step — the only point in a build where every wave has merged and nothing is pending — now emits one more gate that imports the application entrypoint the repo's own tests name (`from app.main import app` and friends) and asserts every HTTP path the plan promised resolves on it, reading the OpenAPI schema and recursing into included routers so FastAPI 0.141's lazy `include_router` is still seen. It runs through the existing gate runner, so it inherits the host / factory-sandbox / k8s-Job / Nix-devshell backends and executes in the same environment the project's own `pytest` gate does; a failure lands in `GATE_FAILURES.md` for the QA/fix loop exactly as the #601 layout and #611f dependency guards do. Kept off correct work: no gate at all when the plan promises no HTTP path or no test names an entrypoint, only loopback URLs are treated as promises (a link to somebody's documentation is not a route this build must serve), and an app that cannot be imported is reported **skipped, never passed** — a lane that never ran must not read quieter than one that failed. Shares the single escape hatch `AIFACTORY_TEST_EVIDENCE_GATE=off` with #851/#1111; no second flag. Coder-path code that runs inside the build Job, so it ships via the `-nix` build image.
...
- **Deploys reach the cluster again: cosign v3 signatures were invisible to Kyverno (Factory#799, Factory#804).** `aifactory` ran two days behind `main` with every CI check green. ArgoCD was reconciling normally and the gitops bump had landed; Kyverno was denying the Deployment at admission with `no signatures found`. #1286 bumped `sigstore/cosign-installer` v3.7.0 -> v4.1.2 on 2026-08-13, taking cosign v2 -> v3, and cosign v3 stores signatures as an OCI 1.1 referring artifact instead of the legacy `sha256-<digest>.sig` tag. `cosign tree` shows the split directly: the last image Kyverno accepted reports "Signatures for an image tag: ...sha256-....sig", the first it rejected reports "artifacts via OCI referrer". Kyverno is not behind — v1.18.2 vendors `sigstore/cosign/v3 v3.0.6` — but its `buildCosignOptions` never sets `ExperimentalOCI11`, which is what gates the referrers lookup, so it reads the legacy tag only. `type: SigstoreBundle` does not rescue it either: tested against the live cluster on a throwaway policy in its own namespace, it still reported "no matching signatures found" including with a `.*` subject regexp, because cosign v3 writes the OLD payload at the NEW location and that matches neither Kyverno verifier. The pin is reverted to v3.7.0 in all three signing workflows with a Dependabot ignore for its majors, since cosign v3 offers no supported way back: `--registry-referrers-mode=legacy` is a fetch mode that does not change where `sign` writes, `--tlog-upload=false` is refused outright, and `--new-bundle-format` warns it will be the only supported format in future versions.

- **A signing change that the cluster cannot verify now fails the build instead of the next deploy (Factory#804).** The existing "Verify signature (self-test)" step stayed green through the entire outage above, and structurally had to: it runs the same cosign that produced the signature, so it verifies a format only that version can read and agrees with itself by construction. A check that cannot fail when its consumer breaks is not a check. `.github/scripts/assert-signature-location.sh` asserts the signature is at the legacy tag — the one place Kyverno looks — and is called after every signing step in deploy, release (all three images) and build-nix. Verified against the two real images before being wired in: it passes the cosign v2 build and fails the cosign v3 one, so it would have gone red on the build that caused the outage rather than surfacing three days later as a stale deployment.
