...
def check_task_state(spec_dir: Path) -> tuple[bool, list[str]]:
    ...
    # Check for implementation_plan.json
    plan_file = spec_dir / "implementation_plan.json"
    if not plan_file.exists():
        issues.append("Missing implementation_plan.json")
        return (True, issues)
    ...
    try:
        ...
        issues.append(f"Cannot read implementation_plan.json: {e}")
        return (True, issues)
...
def fix_task(spec_dir: Path, dry_run: bool = False) -> bool:
    ...
    plan_file = spec_dir / "implementation_plan.json"
    if not plan_file.exists():
        print("  ✗ Cannot fix: missing implementation_plan.json")
        return False
