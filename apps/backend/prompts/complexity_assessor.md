...
- `project_index.json` - Project structure (optional, may be in spec dir or aifactory dir)
...
- No logic changes - just values, text, or style
- Zero risk of breaking anything
- Examples: typo fixes, color changes, text label updates, config value changes
- **Planner recommendation: 1 subtask only**

### SIMPLE
- 1-2 files modified
- Single service
- Minor logic changes allowed
- No external integrations
- No infrastructure changes
- No new dependencies
- Examples: simple bug fixes, add a field, update validation, small UI changes
- **Planner recommendation: 1-3 subtasks**
...
START
  ...
  │     NO ↓
  │
  ├─► Will this touch 2 files OR involve logic changes?
  │     YES → SIMPLE (1-3 subtasks)
  │     NO ↓
  │
  └─► TRIVIAL (single file, no logic change → 1 subtask)
```
...
```json
{
  ...
  "confidence": 0.95,
  "reasoning": "Single file, single CSS change with no logic. Planner should create only 1 subtask.",
  "analysis": {
    "scope": {
      "estimated_files": 1,
  ...
  "validation_recommendations": {
    ...
    "security_scan_required": false,
    "staging_deployment_required": false,
    "reasoning": "Single CSS value change - no functional code modified, no testing needed."
  }
}
```
