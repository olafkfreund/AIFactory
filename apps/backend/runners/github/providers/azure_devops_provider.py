...
class AzureDevOpsProvider:
    ...
    async def fetch_pr(self, number: int) -> PRData:
        ...
        async with self._client() as client:
            ...
            return PRData(
                ...
                author=pr_data.get("createdBy", {}).get("uniqueName") or pr_data.get("createdBy", {}).get("displayName") or "",
                state=state,
    ...
    async def remove_labels(self, issue_or_pr_number: int, labels: list[str]) -> None:
        ...
        try:
            ...
            url = f"{self._base_url}/{self._org}/{self._proj}/_apis/wit/workitems/{issue_or_pr_number}?api-version=7.1"
            patch = [{"op": "add", "path": "/fields/System.Tags", "value": "; ".join(filtered_tags)}]
            headers = self._headers.copy()
            headers["Content-Type"] = "application/json-patch+json"
            async with httpx.AsyncClient(headers=headers, timeout=30.0) as client:
                await client.patch(url, json=patch)
    ...
    def _parse_work_item(self, wi: dict[str, Any]) -> IssueData:
        ...
        assignee = fields.get("System.AssignedTo", {}).get("uniqueName") or fields.get("System.AssignedTo", {}).get("displayName") or ""
        
        return IssueData(
            number=wi["id"],
            title=fields.get("System.Title", ""),
            body=fields.get("System.Description") or "",
            author=fields.get("System.CreatedBy", {}).get("uniqueName") or fields.get("System.CreatedBy", {}).get("displayName") or "",
