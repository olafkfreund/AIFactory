"""
Standalone stdio entrypoint for the aifactory MCP server.
...
2. ``AIFACTORY_SPEC_DIR`` env var (set by AIFactory's CLI / web UI when a
   spec becomes active)
3. ``AIFACTORY_PROJECT_DIR`` / ``CLAUDE_PROJECT_DIR`` env var
...
The ``.aifactory/current-spec`` pointer-file convention floated by an
...
Run directly for debugging:
    ...
    AIFACTORY_SPEC_DIR=/path/to/spec \
      python -m apps.backend.mcp.aifactory_server
...
def _build_spec_dir_resolver(
    ...
    AIFactory's backend can change ``AIFACTORY_SPEC_DIR`` mid-session and
...
def _ensure_backend_on_path() -> None:
    ...
    When invoked via ``python -m apps.backend.mcp.aifactory_server``, Python
    runs this from the repo root and the import path is fine. When invoked
    via the wrapper script with ``-m mcp.aifactory_server`` from inside
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog="aifactory-mcp-server",
        description=(
            "Standalone MCP server exposing AIFactory's spec-management "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "AIFACTORY_SPEC_DIR env var. When neither is set, tools run in "
    ...
    try:
        ...
        sys.stderr.write(f"aifactory MCP server failed: {exc}\n")
        return 1
    return 0
