...
``aifactory mcp-doctor`` prints the catalog × credentials matrix so operators
...
def _check_operator_config_perms() -> str | None:
    ...
    config_path = Path.home() / ".aifactory" / "mcp-credentials.json"
    ...
    if mode & 0o077:
        return (
            ...
            "AIFactory refuses to read it. Fix with: chmod 0600 ~/.aifactory/mcp-credentials.json"
        )
    return None
