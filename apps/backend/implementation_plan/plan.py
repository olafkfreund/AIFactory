...
class ImplementationPlan:
    ...
    def update_status_from_subtasks(self) -> None:
        ...
        """
        # Every subtask in the plan counts (#1176). testing/cicd subtasks were
        # excluded here as downstream "handoff" work while both engines went on
        # dispatching them, so the status said done while the coder was still
    ...
    def get_progress(self) -> dict[str, Any]:
        ...
        # Completion is judged over every subtask (#1176). The old "coder-owned"
        # subset excluded testing/cicd from the DONE count while still counting
        # them in failed_subtasks below — the accounting disagreed with itself,
        # and with both engines, which dispatched them regardless.
        return {
