 ...
 * ``~/.aifactory/.mcp-key``) instead of the host-wide admin token at
 * ``~/.aifactory/.token``.
 *
...
const ORG_STORAGE_KEY = 'aifactory.apiKeys.orgId';
        ...
        <div className="rounded-lg bg-info/10 border border-info/30 p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            {t(
              ...
              'Mint a scoped key, then set $AIFACTORY_MCP_KEY (or drop the value in ~/.aifactory/.mcp-key) on the laptop that runs the stdio MCP. The key is shown once at creation — copy it before closing the panel.',
            )}
          </p>
        </div>
