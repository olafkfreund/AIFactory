...
export function AdvancedSettings({ settings, onSettingsChange, section, version }: AdvancedSettingsProps) {
  ...
  // AI Factory source update state
  ...
  useEffect(() => {
    const cleanup = window.API.onAutoBuildSourceUpdateProgress((progress) => {
  ...
  useEffect(() => {
    const cleanupAvailable = window.API.onAppUpdateAvailable((info) => {
      setAppUpdateInfo(info);
      setIsCheckingAppUpdate(false);
    });

    const cleanupDownloaded = window.API.onAppUpdateDownloaded((info) => {
      setAppUpdateInfo(info);
    ...
    const cleanupProgress = window.API.onAppUpdateProgress((progress) => {
  ...
  const checkForSourceUpdates = async () => {
    ...
    try {
      const result = await window.API.checkAutoBuildSourceUpdate();
  ...
  const handleDownloadSourceUpdate = () => {
    ...
    window.API.downloadAutoBuildSourceUpdate();
  };
                ...
                {sourceUpdateCheck.updateAvailable && (
                  <div className="space-y-4 pt-2">
                    ...
                    {sourceUpdateCheck.releaseUrl && (
                      <button
                        onClick={() => window.API.openExternal(sourceUpdateCheck.releaseUrl!)}
  ...
  // Ensure notifications object exists (fallback to defaults if missing)
