      ...
      <div className="p-4 space-y-2">
        {isLoadingLogs ? (
          ...
          <>
            ...
            {phaseLogs?.phases && (['planning', 'coding', 'validation'] as TaskLogPhase[]).map((phase) => (
              <PhaseLogSection
                key={phase}
                phase={phase}
            ...
            {/* Build Progress section - shows detailed build logs */}
            {phaseLogs?.buildProgress && (
              <BuildProgressSection
                buildProgress={phaseLogs.buildProgress}
              />
            )}
            <div ref={logsEndRef} />
          </>
...
// Build Progress Section Component - displays detailed build logs from build-progress.txt
interface BuildProgressSectionProps {
  buildProgress: string;
}

function BuildProgressSection({ buildProgress }: BuildProgressSectionProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const lineCount = buildProgress.split('\n').length;
  ...
  return (
    <Collapsible open={isExpanded} onOpenChange={setIsExpanded}>
      <CollapsibleTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sm">Build Log</span>
            <span className="text-xs text-muted-foreground">
              ({lineCount} lines)
            </span>
          </div>
      ...
      <CollapsibleContent>
        <div className="mt-1 ml-6 border-l-2 border-border pl-4 py-2">
          <div className="bg-secondary/30 rounded-md border border-border/50 overflow-hidden">
            <pre className="text-[11px] text-muted-foreground whitespace-pre-wrap break-words font-mono p-3 max-h-[500px] overflow-y-auto">
              {buildProgress}
            </pre>
          </div>
        </div>
