...
export function useChangelog() {
  ...
  useEffect(() => {
    const cleanupProgress = window.API.onChangelogGenerationProgress(
      (projectId, progress) => {
    ...
    const cleanupComplete = window.API.onChangelogGenerationComplete(
      (projectId, result) => {
        if (projectId === selectedProjectId) {
    ...
    const cleanupError = window.API.onChangelogGenerationError(
      (projectId, errorMsg) => {
        if (projectId === selectedProjectId) {
          setIsGenerating(false);
          setError(errorMsg);
    ...
    if (selectedProjectId) {
      try {
        ...
        if (sourceMode === 'tasks' && selectedTaskIds.length > 0) {
          ...
          const result = await window.API.suggestChangelogVersion(
            selectedProjectId,
            selectedTaskIds
          );
          ...
          const result = await window.API.suggestChangelogVersionFromCommits(
            selectedProjectId,
            previewCommits
          );
          if (result.success && result.data) {
