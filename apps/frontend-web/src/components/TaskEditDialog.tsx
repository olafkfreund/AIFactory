...
export function TaskEditDialog({ task, open, onOpenChange, onSaved }: TaskEditDialogProps) {
  ...
  const [mode, setMode] = useState<'quick' | 'full'>(task.metadata?.mode || 'full');
  ...
  const handleSave = async () => {
    ...
    // Determine original profile ID for comparison
    const originalIsAutoProfile = task.metadata?.isAutoProfile;
    const originalModel = task.metadata?.model;
    const originalThinking = task.metadata?.thinkingLevel;
    let originalProfileId: string;
    if (originalIsAutoProfile) {
      originalProfileId = 'auto';
    } else if (originalModel && originalThinking) {
      const matchingProfile = DEFAULT_AGENT_PROFILES.find(
        p => p.model === originalModel && p.thinkingLevel === originalThinking && !p.isAutoProfile
      );
      originalProfileId = matchingProfile?.id || 'custom';
    } else {
      originalProfileId = settings.selectedAgentProfile || 'auto';
    }
    ...
    const hasChanges =
      ...
      (profileId === 'auto' && JSON.stringify(phaseThinking) !== JSON.stringify(task.metadata?.phaseThinking || DEFAULT_PHASE_THINKING));
    ...
    // Build metadata updates - use null to explicitly clear fields on backend
    // We use Record<string, unknown> to allow null values that signal field clearing
    const metadataUpdates: Record<string, unknown> = {};

    // Classification fields - use null to clear, value to set
    metadataUpdates.category = category || null;
    metadataUpdates.priority = priority || null;
    metadataUpdates.complexity = complexity || null;
    metadataUpdates.impact = impact || null;
    ...
    if (profileId === 'auto') {
      ...
      metadataUpdates.phaseModels = phaseModels || null;
      metadataUpdates.phaseThinking = phaseThinking || null;
    } else {
      // Clear auto profile fields when switching away from auto
      metadataUpdates.isAutoProfile = false;
      metadataUpdates.phaseModels = null;
      metadataUpdates.phaseThinking = null;
    }
