...
// Recommended embedding models for AI Factory Memory
...
export function OllamaModelSelector({
  ...
  const checkInstalledModels = async (abortSignal?: AbortSignal) => {
    ...
    try {
      ...
      const installResult = await window.API.checkOllamaInstalled();
      ...
      const statusResult = await window.API.checkOllamaStatus();
      if (abortSignal?.aborted) return;
      ...
      const result = await window.API.listOllamaEmbeddingModels();
      if (abortSignal?.aborted) return;
