...
export function Worktrees({ projectId }: WorktreesProps) {
  ...
  // - Exclude worktrees without a related task (deleted tasks)
  // - Exclude worktrees where task is archived
  // - Sort by task updatedAt date descending (newest first)
  const filteredAndSortedWorktrees = useMemo(() => {
    return worktrees
      .filter((worktree) => {
        const task = findTaskForWorktree(worktree.specName);
        // Exclude if no task found or task is archived
        return task && !task.metadata?.archivedAt;
      })
      .sort((a, b) => {
        const taskA = findTaskForWorktree(a.specName);
        const taskB = findTaskForWorktree(b.specName);

        // Sort by updatedAt descending (newest first)
        const dateA = taskA?.updatedAt ? new Date(taskA.updatedAt).getTime() : 0;
        const dateB = taskB?.updatedAt ? new Date(taskB.updatedAt).getTime() : 0;
        return dateB - dateA;
      });
  }, [worktrees, tasks]);

  // Compute list of empty worktrees (0 commits and 0 files changed)
  const emptyWorktrees = useMemo(() => {
    return filteredAndSortedWorktrees.filter(
      (wt) => wt.commitCount === 0 && wt.filesChanged === 0
    );
  }, [filteredAndSortedWorktrees]);

  // Handle cleanup of empty worktrees
  const handleCleanupEmpty = async () => {
    if (emptyWorktrees.length === 0) return;

    setIsCleaningUp(true);
    setError(null);
    try {
      for (const worktree of emptyWorktrees) {
        const task = findTaskForWorktree(worktree.specName);
        if (task) {
          await window.API.discardWorktree(task.id);
        }
      }
      await loadWorktrees(); // Refresh list
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to cleanup worktrees');
    } finally {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      const result = await window.API.mergeWorktree(task.specId);
      if (result.success && result.data) {
  ...
  return (
    <div className="flex h-full flex-col p-6">
        ...
        <div className="flex gap-2">
          {emptyWorktrees.length > 0 && (
            ...
            >
              ...
              Cleanup Empty ({emptyWorktrees.length})
            </Button>
          )}
          <Button
            variant="outline"
      ...
      {/* Empty state */}
      {!isLoading && filteredAndSortedWorktrees.length === 0 && (
      ...
      {/* Worktrees list */}
      {filteredAndSortedWorktrees.length > 0 && (
