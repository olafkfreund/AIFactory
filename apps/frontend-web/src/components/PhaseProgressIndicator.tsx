...
interface PhaseProgressIndicatorProps {
  ...
  isRunning?: boolean;
  className?: string;
  /** Live progress from backend (overrides subtask-based calculation when provided) */
...
const PHASE_COLORS: Record<ExecutionPhase, { color: string; bgColor: string }> = {
  ...
  spec_creation: { color: 'bg-amber-500', bgColor: 'bg-amber-500/20' },  // Spec creation (orange like planning)
  planning: { color: 'bg-amber-500', bgColor: 'bg-amber-500/20' },
  plan_review: { color: 'bg-yellow-500', bgColor: 'bg-yellow-500/20' },  // Waiting for human approval
...
export const PhaseProgressIndicator = memo(function PhaseProgressIndicator({
  ...
  const subtaskBasedProgress = totalSubtasks > 0 ? Math.round((completedSubtasks / totalSubtasks) * 100) : 0;
  // Use live overallProgress from backend if provided, otherwise fall back to subtask calculation
  const subtaskProgress = overallProgress ?? subtaskBasedProgress;
  ...
  const getActivePhaseEntries = () => {
    if (phase === 'spec_creation' || phase === 'planning') return planningEntries;
