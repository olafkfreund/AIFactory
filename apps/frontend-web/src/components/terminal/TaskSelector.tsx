...
import { cn, extractTaskNumber } from '../../lib/utils';
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-56">
          ...
          <div className="px-2 py-1 text-sm font-medium truncate">
            ...
            {(() => {
              const number = extractTaskNumber(associatedTask.specId);
              return number ? <span className="text-muted-foreground"> ({number})</span> : null;
            })()}
          </div>
              ...
              {backlogTasks.filter(t => t.id !== associatedTask.id).slice(0, 5).map((task) => (
                ...
                >
                  ...
                  <span className="truncate">
                    {task.title}
                    {(() => {
                      const number = extractTaskNumber(task.specId);
                      return number ? <span className="text-muted-foreground"> ({number})</span> : null;
                    })()}
                  </span>
                </DropdownMenuItem>
