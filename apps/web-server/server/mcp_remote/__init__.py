...
Sister of the stdio MCP server at ``apps/backend/mcp_server/aifactory_server.py``.
...
calls — can observe and direct AIFactory regardless of which LLM is doing the
work underneath.

Opt-in only. Set ``AIFACTORY_MCP_REMOTE_ENABLED=true`` to mount; default off
...
def is_enabled() -> bool:
    ...
    `AIFACTORY_MCP_REMOTE_ENABLED=true` is unambiguous in deployment manifests.
    """
    return os.environ.get("AIFACTORY_MCP_REMOTE_ENABLED", "").lower() in (
        "true",
        "1",
        "yes",
        "on",
