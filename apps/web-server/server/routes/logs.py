...
async def receive_frontend_logs(request: FrontendLogsRequest):
    ...
    try:
        ...
        # This route is in TokenAuthMiddleware.PUBLIC_PATHS: anyone who can
        # reach the server can POST here WITHOUT AUTHENTICATING. An OSError
        # renders the absolute log path, so this was disclosure to an
        # unauthenticated caller (Factory#718).
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=client_error(logger, "Failed to write frontend logs", e),
        ) from e
...
async def get_raw_logs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=client_error(logger, "Failed to read log file", e),
        ) from e
...
async def download_logs(log_type: str):
    ...
    if not log_file.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Log file does not exist"
        )
...
async def clear_all_logs():
    ...
    if success:
        return ClearLogsResponse(
            success=True, message="All log files cleared successfully"
        )
    else:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
