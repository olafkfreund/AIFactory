...
uid <N>``).  We default the socket to a user-owned path:
  ...
  1. ``$XDG_RUNTIME_DIR/aifactory-rmux/sock`` if the env var is set
     (standard on systemd user sessions; ``/run/user/<uid>/`` is owned
     by the user and tmpfs-backed)
  2. ``~/.cache/aifactory/rmux/sock`` as a portable fallback

Operators can override with the ``socket_dir`` constructor arg — the
production container uses ``/var/run/aifactory/rmux/`` via an
...
def _default_socket_dir() -> Path:
    ...
    if xdg:
        return Path(xdg) / "aifactory-rmux"
    return Path.home() / ".cache" / "aifactory" / "rmux"
