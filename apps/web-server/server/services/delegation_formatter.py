"""Render the AIFactory-enriched plan as a GitHub issue comment.
...
Output shape (markdown):
    ## :sparkles: AIFactory enrichment
    ### Spec
    ...spec.md body, trimmed...
    ### Acceptance criteria
    ...derived from implementation_plan.json subtasks...
    ### Affected files
    ...derived from implementation_plan.json...
    ### Implementation plan
...
def render_plan_as_comment(plan_json_path: Path, spec_md_path: Path) -> str:
    ...
    parts: list[str] = ["## ✨ AIFactory enrichment", ""]
    ...
    parts.append("---")
    parts.append("_AIFactory drafted this plan. @Copilot — please implement._")
    return "\n".join(parts)
