...
2. **Await** that subprocess to completion (gap #3 from issue #144 — the
   ...
   the enrichment comment was rendered before ``implementation_plan.json``
   was written).
3. Render the AIFactory enrichment comment.
...
ENRICHMENT_MARKER = "## ✨ AIFactory enrichment"

# How long to wait for the planner subprocess to write
# implementation_plan.json before giving up and posting whatever we have.
...
async def _existing_enrichment_comment(provider: Any, issue_number: int) -> int | None:
    """Return the comment ID of any prior AIFactory enrichment comment
