...
Configuration (env vars, read at backend construction):
  AZURE_KEYVAULT_URL    — required. e.g. ``https://kv-aifactory.vault.azure.net``
  AZURE_KEYVAULT_KEY    — required. Name (and optional version) of the
                          key. Examples: ``aifactory-root`` or
                          ``aifactory-root/abc123`` for a pinned version.
    ...
    def from_env(cls) -> AzureKeyVaultBackend:
        ...
        if not url:
            raise RuntimeError(
                ...
                "(e.g. https://kv-aifactory.vault.azure.net)."
            )
