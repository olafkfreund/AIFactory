"""KMS backend abstraction for AIFactory's at-rest encryption.
...
# statement that credentials must be encrypted at rest (#1290).
...
def encryption_is_required() -> bool:
    ...
    broken" (#1290), and callers use it to decide whether degrading to
...
def enforce_kms_safety() -> None:
    ...
    Called only from the real server entrypoint (``server.main.__main__``),
    mirroring ``config.enforce_disable_auth_safety`` /
    ``config.enforce_cors_safety`` — TestClient never reaches it.
