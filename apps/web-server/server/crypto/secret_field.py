"""Seal/unseal a secret string that lives inside a JSON store on disk (#1276).
...
in **AIFactory 1.5.0**. By then every live store has been rewritten many times
...
def seal(value: str | None) -> str | None:
    ...
    When a backend WAS selected (#1290) the fallback is a lie: the operator
    believes these credentials are encrypted. There we raise, so the save fails
    loudly and retryably instead of quietly writing a token in the clear.
    """
...
def seal_fields(
    data: JsonStore, keys: tuple[str, ...] = APP_SETTINGS_SECRET_KEYS
) -> JsonStore:
