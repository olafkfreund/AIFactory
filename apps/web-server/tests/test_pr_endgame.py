...
def test_read_review_verdict_copilot_aware():
    ...
    rs = pe.read_review_verdict("o", "r", 1, runner=r)
    assert (
        rs.verdict == "approved" and not rs.copilot_approved and not rs.copilot_reviewed
    )

    r = FakeRunner(
        {"reviews": _reviews(("CHANGES_REQUESTED", COPILOT), ("APPROVED", "alice"))}
    )
    rs = pe.read_review_verdict("o", "r", 1, runner=r)
    ...
    r = FakeRunner(
        {
            ...
            "reviews": _reviews(
                ("APPROVED", COPILOT)
            ),  # Copilot approved → gate satisfied
            "pr merge": CmdResult(0, "merged", ""),
        }
    )
    res = asyncio.run(
...
def test_verdict_from_review_result():
    ...
    assert (
        pe.verdict_from_review_result({"verdict": "ready_to_merge"}).verdict
        == "approved"
    )
    assert (
        pe.verdict_from_review_result({"verdict": "needs_revision"}).verdict
        == "changes_requested"
    )
    assert (
        pe.verdict_from_review_result({"verdict": "blocked"}).verdict
        == "changes_requested"
    )
    assert (
        pe.verdict_from_review_result({"verdict": "merge_with_changes"}).verdict
        == "changes_requested"
    )
    # generic synonyms
    assert pe.verdict_from_review_result({"verdict": "approve"}).verdict == "approved"
    assert (
        pe.verdict_from_review_result({"verdict": "request_changes"}).verdict
        == "changes_requested"
    )
    # blockers field forces changes_requested + carries them as findings
    rs = pe.verdict_from_review_result(
        {"verdict": "ready_to_merge", "blockers": [{"title": "b"}]}
    )
    ...
    assert (
        pe.verdict_from_review_result({"findings": [{"severity": "high"}]}).verdict
        == "changes_requested"
    )
    # tolerate {data:{...}} wrapper
    assert (
        pe.verdict_from_review_result({"data": {"verdict": "ready_to_merge"}}).verdict
        == "approved"
    )
...
def test_fix_loop_fixes_then_merges():
    ...
    states = [
        pe.ReviewState(
            "changes_requested", findings=[{"severity": "high", "title": "x"}]
        ),
...
class _SeqRunner:
    ...
    def __call__(self, argv, cwd=None):
        ...
        if "update-branch" in j:
            return CmdResult(
                0 if self.update_ok else 1, "", "" if self.update_ok else "conflicts"
            )
        if "pr merge" in j:
            self._merges += 1
            if self._merges == 1:
                return CmdResult(
                    1, "", "not mergeable: the merge commit cannot be cleanly created"
                )
            return CmdResult(0, "merged", "")  # after update-branch
        return CmdResult(0, "", "")

    def saw(self, n):
        return any(n in c for c in self.calls)
