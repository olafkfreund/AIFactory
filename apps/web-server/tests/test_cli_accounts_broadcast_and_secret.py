...
* ``_broadcast_cli_auth_event`` ran the WebSocket broadcast on an event loop
  ...
  connections owned by the ASGI server's loop. The error is swallowed by
  ``deliver_local``, which then unregisters the client -- a silent drop.
...
from server.websockets import event_bus  # noqa: E402
...
async def test_worker_thread_broadcast_reaches_the_connected_websocket() -> None:
    ...
    the worker thread. ``deliver_local`` swallows send errors and unregisters
    the client, so the browser never heard that the CLI login completed and
    the portal showed it as still pending -- with nothing in the response to
    say so.
    ...
    event_bus.register_client(ws, None)
    try:
        ...
        assert ws in event_bus.active_connections, (
            "the client was unregistered by a failed send"
        )
    finally:
        event_bus.unregister_client(ws)
...
def test_credentials_file_is_never_readable_even_for_an_instant(
    ...
    * fixed: ``mkstemp`` creates at 0600 regardless of umask and
      ``os.replace`` publishes it, so there is no ``chmod`` at all.
    ...
    assert not chmods, (
        ...
        f"was created at the umask default (0666 here) with the token already "
...
def test_credentials_write_replaces_rather_than_truncates(
    ...
    ``write_text`` truncates the destination in place. The fixed write goes to
    a temp file and is published with ``os.replace``, so the destination inode
    is swapped, never emptied: an open handle on the old file keeps reading
    the old, complete content.
    """
    dest = tmp_path / "codex-credentials.json"
    monkeypatch.setitem(ca.CLI_CONFIG["codex"], "stored_credentials", dest)
    monkeypatch.setattr(ca, "CREDENTIALS_DIR", tmp_path)
