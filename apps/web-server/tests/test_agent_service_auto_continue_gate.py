"""`start_task_execution(auto_continue=False)` must not crash (CodeQL unflagged -- see below).
...
Mutation check: re-indent lines 880-900 of `agent_service.py` back under
...
async def test_start_task_execution_reaches_the_spawn_on_both_auto_continue_settings(
    ...
    (tmp_path / ".aifactory" / "specs" / "spec1").mkdir(parents=True)
    ...
    assert service._spec_dirs["t1"] == tmp_path / ".aifactory" / "specs" / "spec1"
...
async def test_skill_context_is_written_regardless_of_auto_continue(
    ...
    write_skill_context.assert_called_once_with(
        tmp_path / ".aifactory" / "specs" / "spec1"
    )
