...
it verbatim on every surface::
    ...
    >>> repr(CreateGitCredentialRequest(provider="gitlab", token="glpat-SUPERSECRET123", ...))
...
* :func:`test_no_credential_field_renders_its_secret` is the generic one. It
  parses every module under ``apps/web-server/server`` with :mod:`ast` -- no
  ...
  remembering this file exists. Pinning today's models by name would pass
  forever while the next ``token: str`` sailed in.
...
WHICH FIX APPLIES WHERE
    ...
    * ``AppSettings`` / ``SettingsUpdate`` / ``ClaudeProfile`` / ``ApiProfileUpdate``
      / ``SourceEnvUpdate`` / ``ProjectEnvUpdate`` / ``ProjectSettings*`` /
      ``AutoFixConfigPayload`` are PERSISTED by dumping them --
      ``save_app_settings`` writes ``model_dump_json()`` straight to disk. Under
      ``SecretStr`` the first save would overwrite every stored credential with
      ``"**********"``: silent, permanent credential loss.
...
from server.routes.auth_routes import AuthResponse, LoginRequest, RegisterRequest
from server.routes.git_credentials import CreateGitCredentialRequest
...
# Names here are camelCase as often as snake_case (`githubToken`, `apiKey`), and
# a snake_case-only split silently misses every one of them -- which is how the
# original issue's grep under-counted this surface by roughly threefold.
_WORD = __import__("re").compile(r"[A-Z]+(?![a-z])|[A-Z][a-z]*|[a-z0-9]+")
...
def _server_modules() -> list[Path]:
    return sorted(p for p in _SERVER_DIR.rglob("*.py") if "__pycache__" not in p.parts)


def test_server_tree_is_present() -> None:
    """Fail loudly rather than passing vacuously if the tree moves."""
    assert _server_modules(), f"no modules found under {_SERVER_DIR}"
        ...
        for node in ast.walk(tree):
            ...
            for stmt in node.body:
                ...
                if "SecretStr" in ast.unparse(stmt.annotation) or _has_repr_false(
                    ...
                    continue
                offenders.append(
                    f"{module.relative_to(_SERVER_DIR)}:{stmt.lineno} {node.name}.{field} "
...
def test_login_and_register_mask_the_password() -> None:
    ...
    # _hash_password / _verify_password must still see the real password.
