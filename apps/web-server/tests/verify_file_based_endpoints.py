...
class EndpointVerifier:
    ...
    def verify_endpoint(
        ...
        if (
            f"def {function_name}" not in content
            and f"async def {function_name}" not in content
        ):
        ...
        if function_match:
            ...
            function_body_clean = re.sub(
                r'""".*?"""', "", function_body_clean, flags=re.DOTALL
            )
            ...
            for pattern in stub_patterns:
                if re.search(pattern, function_body_clean):
                    ...
                    lines = [
                        line.strip()
                        for line in function_body_clean.split("\n")
                        if line.strip()
                    ]
            ...
            if has_validation and (has_file_io or "file-based" in description.lower()):
                ...
                self.issues.append(
                    f"⚠️  {function_name} in {file_name} may be incomplete"
                )
                self.results.append(
                    {
                        "phase": phase,
    ...
    def verify_phase_5(self):
        ...
        self.verify_endpoint(
            "roadmap.py", "delete_idea", "5.3", "Remove idea from ideation.json"
        )
