...
def test_per_tenant_anchor_unique_per_org_day(test_postgres_url: str) -> None:
    ...
    with engine.begin() as conn:
        ...
        existing_key = conn.execute(
            text("SELECT MIN(version) FROM audit_signing_keys WHERE org_id = :oid"),
            {"oid": org_id},
        ).scalar()
...
def test_shared_anchor_still_allowed_after_migration(test_postgres_url: str) -> None:
    ...
    with engine.begin() as conn:
        ...
        existing = conn.execute(
            text("SELECT MIN(version) FROM audit_signing_keys WHERE org_id IS NULL")
        ).scalar()
        if existing is None:
            conn.execute(
                text("""
        ...
        org_id = conn.execute(
            text("SELECT org_id FROM audit_anchors WHERE id='shared-test-1'")
        ).scalar()
