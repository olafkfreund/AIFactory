...
This test imports main.create_app() and asserts:
  ...
  - The FastAPI app's title is the AIFactory brand, not Magestic.
...
@pytest.mark.obs
def test_main_app_title_is_aifactory(real_app) -> None:
    """Regression: app title is AIFactory, not the old Magestic brand."""
    assert real_app.title == "AIFactory Web API", (
        f"Expected 'AIFactory Web API', got {real_app.title!r}"
    )
    # description should mention AIFactory, not Magestic.
    assert "Magestic" not in (real_app.description or "")
    assert "AIFactory" in (real_app.description or "")
