...
def test_health_endpoint_responds(
    built_image: str, container_name: str, free_port: int
) -> None:
...
@pytest.mark.docker
@pytest.mark.slow
def test_no_net_admin_required(
    built_image: str, container_name: str, free_port: int
) -> None:
...
def test_no_entrypoint_shell_script(built_image: str) -> None:
    ...
    result = docker_run(
        built_image, "ls", "/usr/local/bin/docker-entrypoint.sh", timeout=10
    )
...
@pytest.mark.docker
@pytest.mark.slow
def test_runs_with_read_only_root_fs(
    built_image: str, container_name: str, free_port: int
) -> None:
