"""
Smoke tests for the standalone aifactory MCP server.

Issue #10 — Epic #6. Three layers:

1. **Module import** — ``apps.backend.mcp.aifactory_server`` imports cleanly
...
class TestModuleImports:
    def test_aifactory_server_module_imports(self) -> None:
        """The standalone entrypoint must import without dragging in heavy deps."""
        sys.path.insert(0, str(_BACKEND))
        try:
            from mcp_server import aifactory_server  # noqa: F401
        finally:
            sys.path.pop(0)
    ...
    def test_main_callable(self) -> None:
        sys.path.insert(0, str(_BACKEND))
        try:
            from mcp_server.aifactory_server import main
        finally:
            sys.path.pop(0)
...
def mcp_subprocess(tmp_path: Path):
    ...
    env = os.environ.copy()
    env["AIFACTORY_SPEC_DIR"] = str(tmp_path)  # degraded "no active spec" mode
    env["PYTHONUNBUFFERED"] = "1"

    proc = subprocess.Popen(
        [sys.executable, "-m", "mcp_server.aifactory_server"],
...
class TestStdioJsonRpc:
    def test_initialize_and_list_seven_tools(
        ...
        assert init_response["result"]["serverInfo"]["name"] == "aifactory"
...
class TestMcpJsonSchema:
    ...
    def test_aifactory_server_registered(self) -> None:
        ...
        args = entry.get("args", [])
        assert any("start-aifactory-mcp.sh" in str(a) for a in args), (
            f"args must reference the wrapper script; got {args}"
        )

    def test_wrapper_script_present_and_executable(self) -> None:
        script = _REPO_ROOT / "scripts" / "start-aifactory-mcp.sh"
        assert script.exists(), "scripts/start-aifactory-mcp.sh missing"
        assert os.access(script, os.X_OK), (
            "scripts/start-aifactory-mcp.sh must have the executable bit set "
            "(run `chmod +x scripts/start-aifactory-mcp.sh`)"
        )
