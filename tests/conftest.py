...
def pytest_runtest_setup(item):
    ...
    module_mocks = {
        ...
        "test_qa_report_project_detection": {
            "claude_agent_sdk",
            "ui",
            "progress",
            "task_logger",
    ...
    # mocks take effect.
    if cleaned_up and module_name in (
        "test_qa_loop",
        "test_review",
        "test_qa_criteria",
    ):
...
try:
    ...
    def _make_skip_fixture(name: str):
        ...
        def _skipped():
            _pytest.skip(
                f"FastAPI not installed – endpoint fixture '{name}' unavailable"
            )
