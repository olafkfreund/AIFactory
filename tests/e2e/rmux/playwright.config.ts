 ...
 *   AIFACTORY_E2E_PROJECT_ID=8aad9e7f-... \
...
export default defineConfig({
  ...
  use: {
    baseURL: process.env.AIFACTORY_E2E_BASE_URL ?? 'http://localhost:3100',
    trace: 'retain-on-failure',
    screenshot: 'only-on-failure',
    video: 'retain-on-failure',
    // Bearer token to drive the API and as the WS query param.  The
    // ``AIFACTORY_E2E_TOKEN`` env var is set by the CI job from
    // ``~/.aifactory/.token`` (a file the web-server writes at boot).
    extraHTTPHeaders: process.env.AIFACTORY_E2E_TOKEN
      ? { Authorization: `Bearer ${process.env.AIFACTORY_E2E_TOKEN}` }
      : undefined,
  },
