"""SSRF guard on the per-project ``gitBaseUrl`` setting (#1360).
...
Ported from TFactory#1116 and PFactory#611, which closed the identical defect
there.
"""
...
def _project_with_base_url(monkeypatch, provider: str, base_url: str, tmp_path) -> None:
    ...
    Both readers import ``load_projects`` from ``server.project_registry`` inside
    the function body, so patching the registry module covers both.
    """
    from server import project_registry
    ...
    monkeypatch.setattr(project_registry, "load_projects", lambda: record)
...
def test_routes_github_reader_accepts_self_hosted(monkeypatch, tmp_path):
    ...
    assert _get_project_provider("p1")._base_url == "https://10.1.2.3"
