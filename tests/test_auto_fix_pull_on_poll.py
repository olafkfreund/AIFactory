...
async def test_pull_on_poll_fast_forwards_cloned_projects(tmp_path):
    ...
    project_path = tmp_path / "olaf-AIFactory"
    project_path.mkdir()
    ...
    projects_fixture = {
        "proj-cloned": {
            ...
            "name": "AIFactory",
            "clonedFrom": "https://github.com/olaf/AIFactory.git",
            "clonedBranch": "main",
        }
    }
    ...
    assert kwargs["git_url"] == "https://github.com/olaf/AIFactory.git"
    assert kwargs["branch"] == "main"
    assert kwargs["slug"] == "olaf-AIFactory"
...
async def test_add_project_persists_cloned_from(tmp_path, monkeypatch):
    ...
    # `add_project` lives in `routes.projects`, which binds these names in its
    # OWN namespace at import time, so that is where the patch has to land --
    # not on `server.project_registry`, which only the call-time importers read.
    ...
    project = ProjectCreate(
        gitUrl="https://github.com/olaf/AIFactory.git", branch="dev"
    )
    ...
    assert only_project["clonedFrom"] == "https://github.com/olaf/AIFactory.git"
