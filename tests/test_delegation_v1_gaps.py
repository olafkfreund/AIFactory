...
def test_write_spec_dir_injects_delegation_when_default_on(tmp_path: Path):
    ...
    req = json.loads((tmp_path / ".aifactory" / "specs" / spec_name / "requirements.json").read_text())
...
async def test_run_delegation_awaits_planner_before_reading_plan(tmp_path: Path):
    ...
    the planner has had a chance to write implementation_plan.json."""
    from server.services import delegation_runner

    project_path = tmp_path / "proj"
    spec_dir = project_path / ".aifactory" / "specs" / "001-gh42-test"
    spec_dir.mkdir(parents=True)
    (spec_dir / "spec.md").write_text("# Title\n\nBody")

    # The planner writes implementation_plan.json from inside wait()
    ...
    async def _fake_wait():
        ...
        (spec_dir / "implementation_plan.json").write_text(json.dumps(plan_payload))
        return 0
...
async def test_run_delegation_planner_timeout_does_not_block(tmp_path: Path, monkeypatch):
    ...
    project_path = tmp_path / "proj"
    spec_dir = project_path / ".aifactory" / "specs" / "001-gh99-hang"
    spec_dir.mkdir(parents=True)
...
async def test_run_delegation_skips_comment_if_marker_already_present(
    ...
    project_path = tmp_path / "proj"
    spec_dir = project_path / ".aifactory" / "specs" / "001-gh42-dup"
    spec_dir.mkdir(parents=True)
    (spec_dir / "spec.md").write_text("# Title\n\nBody")
