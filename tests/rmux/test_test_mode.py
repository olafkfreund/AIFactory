"""Tests for the ``AIFACTORY_TEST_AGENT_CMD`` override in
...
def agent_service(tmp_path, monkeypatch):
    ...
    (project / ".aifactory" / "specs" / "001-test").mkdir(parents=True)
...
class TestTestAgentCmdOverride:
    ...
    async def test_no_override_uses_python_runpy(
        ...
        svc, project, spec_id = agent_service
        monkeypatch.delenv("AIFACTORY_TEST_AGENT_CMD", raising=False)
    ...
    async def test_empty_override_falls_back_to_runpy(
        ...
        """Whitespace-only AIFACTORY_TEST_AGENT_CMD is treated as unset
        so prod misconfigurations don't accidentally noop the agent."""
        svc, project, spec_id = agent_service
        monkeypatch.setenv("AIFACTORY_TEST_AGENT_CMD", "   ")
