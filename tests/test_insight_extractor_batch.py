...
class TestThreshold:
    async def test_below_threshold_uses_sequential_path(self, monkeypatch, tmp_path):
        ...
        monkeypatch.delenv("AIFACTORY_BATCH_MIN_JOBS", raising=False)
        monkeypatch.delenv("AIFACTORY_BATCH_DISABLE", raising=False)
    ...
    async def test_at_threshold_uses_batch_path(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch(
                "core.batch.submit_batch", AsyncMock(return_value="msgbatch_test")
            ) as submit_mock,
            patch(
                "core.batch.await_batch",
                AsyncMock(
                    return_value=[
                        SimpleNamespace(
                            custom_id="t1",
    ...
    async def test_kill_switch_forces_sequential(self, monkeypatch, tmp_path):
        """AIFACTORY_BATCH_DISABLE=1 forces sequential even at high N."""
        monkeypatch.setenv("AIFACTORY_BATCH_DISABLE", "1")
...
class TestFailurePaths:
    async def test_partial_failure_falls_back_to_generic_per_entry(
        ...
        with (
            ...
            patch.object(ie, "_build_extraction_prompt", return_value="extract"),
    ...
    async def test_timeout_falls_back_to_sequential_for_all(
        self, monkeypatch, tmp_path
    ):
        monkeypatch.delenv("AIFACTORY_BATCH_DISABLE", raising=False)
        ...
        with (
            ...
            patch.object(ie, "extract_session_insights", seq_mock),
        ):
            results = await ie.extract_session_insights_bulk(
    ...
    async def test_missing_api_key_falls_back_to_sequential(
        self, monkeypatch, tmp_path
    ):
