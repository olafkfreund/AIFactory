...
def isolate_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mc, "OPERATOR_CONFIG_PATH", tmp_path / ".aifactory" / "mcp-credentials.json")
...
def _write_operator_config(tmp_path: Path, payload: dict) -> Path:
    """Write a 0600 ~/.aifactory/mcp-credentials.json and return its path."""
    target = tmp_path / ".aifactory" / "mcp-credentials.json"
