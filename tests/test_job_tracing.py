"""Unit tests for the job/agent-side OTel bootstrap (Epic #35 #42 PR-2).

The build Job — and the agent subprocess — inherit ``TRACEPARENT`` from
``job_dispatch.trace_env`` / ``make_subprocess_env`` and run
``init_agent_tracing()`` at startup to join that trace and emit. These tests
cover both branches (env set / env unset) and the failure-safe contract.

The module under test is ``core/job_tracing.py``, a byte-exact vendored copy of
the hub canonical ``scripts/job_tracing.py`` (Factory#638) — it was AIFactory's
``core/tracing_bootstrap.py`` until TFactory's verify Job needed the same
emitter and a second hand-copy would have been two forks of one file with no
gate over either. Never edit the vendored copy here; change the hub canonical,
re-vendor, bump HUB_PIN_SHA.
"""
...
def test_valid_traceparent_env_attaches_context(monkeypatch):
    ...
    assert job_tracing._attach_token is not None
...
def test_get_inherited_traceparent_returns_env_value(monkeypatch):
    ...
    monkeypatch.setenv("TRACEPARENT", SAMPLE_TP)
    assert job_tracing.get_inherited_traceparent() == SAMPLE_TP


def test_get_inherited_traceparent_returns_none_when_unset(monkeypatch):
    monkeypatch.delenv("TRACEPARENT", raising=False)
    assert job_tracing.get_inherited_traceparent() is None
...
def test_a_span_is_actually_opened_and_parented(monkeypatch):
    ...
    assert span is job_tracing._job_span
...
def test_exit_flush_is_bounded_and_does_not_shut_down(monkeypatch):
    ...
    monkeypatch.setattr(job_tracing, "_provider", _StubProvider())
    monkeypatch.setattr(job_tracing, "_job_span", _StubSpan())
    job_tracing._flush_at_exit()

    assert _StubSpan.ended, "the job span must be ended before the flush"
    assert calls["flush"] == job_tracing._FLUSH_TIMEOUT_MS
    ...
    assert job_tracing._EXPORT_TIMEOUT_SECONDS <= 3.0
...
def test_exporter_is_installed_when_an_endpoint_is_configured(monkeypatch):
    ...
    job_tracing._install_exporter(_StubProvider())
    assert len(added) == 1
