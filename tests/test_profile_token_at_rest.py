"""OAuth tokens / API keys in the JSON stores must not be plaintext at rest (#1276).
...
_pool = importlib.import_module("server.services.claude_token_pool")
...
def assert_absent(blob: bytes, secret: str) -> None:
    ...
    for i in range(len(secret) - WINDOW + 1):
        ...
        assert window not in text, (
            ...
            f"stored file (window index {i}, secret length {len(secret)})"
        )
...
def write_legacy_store(data_dir: Path) -> Path:
    """A claude-profiles.json exactly as a pre-#1276 release wrote it."""
...
def test_legacy_plaintext_store_still_loads(store: tuple[Any, Path]) -> None:
    """Read-both: a store written before #1276 keeps working untouched."""
    settings_routes, data_dir = store
...
def test_legacy_plaintext_store_is_upgraded_on_next_write(
    store: tuple[Any, Path],
) -> None:
    ...
    raw = path.read_bytes()
    assert_absent(raw, LEGACY_TOKEN)
    assert settings_routes.load_profiles()["profiles"][0]["oauthToken"] == LEGACY_TOKEN


def test_every_profile_reader_sees_plaintext(store: tuple[Any, Path]) -> None:
    """No regression: the readers outside settings.py resolve a usable token."""
    settings_routes, data_dir = store
    settings_routes.save_profiles(
        {
            "profiles": [{"id": "p1", "name": "Work", "oauthToken": NEW_TOKEN}],
    ...
    os.environ.pop("CLAUDE_CODE_OAUTH_TOKEN_POOL", None)

    creds = _pool._discover_credentials(str(data_dir), None)
    assert [c.token for c in creds] == [NEW_TOKEN]
...
# --- #1290: a SELECTED backend that cannot construct must never degrade -----
#
...
def test_seal_raises_rather_than_writing_plaintext_when_a_backend_is_selected(
    selected_but_broken: None,
) -> None:
