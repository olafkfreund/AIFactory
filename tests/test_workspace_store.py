...
def project_workspace(tmp_path: Path) -> Path:
    ...
    (d / ".aifactory" / "specs" / "001-add-auth" / "implementation_plan.json").write_text(
        json.dumps({"phases": []})
    )
    ...
    (d / ".git" / "objects" / "ab" / "cdef1234567890").write_bytes(b"\x00\x01\x02BINARY")
...
def test_from_settings_reads_env(monkeypatch):
    ...
    monkeypatch.setattr(
        get_settings(), "WORKSPACE_S3_URI_BASE", "s3://test-bucket/wsp",
    )
...
async def test_upload_no_op_when_local_only(project_workspace):
    ...
    await s.upload_project(
        org_id="org-1", project_id="proj-1", local_path=project_workspace,
    )
...
async def test_download_returns_false_when_local_only(tmp_path):
    ...
    ok = await s.download_project(
        org_id="org-1", project_id="proj-1", local_path=tmp_path / "dst",
    )
    assert ok is False
...
async def test_upload_writes_manifest_with_expected_fields(
    store, remote_root, project_workspace,
):
...
async def test_project_exists_after_upload(store, project_workspace):
    ...
    await store.upload_project(
        org_id="o", project_id="p", local_path=project_workspace,
    )
...
async def test_download_restores_file_tree_byte_for_byte(
    store, project_workspace, tmp_path,
):
    ...
    ok = await store.download_project(
        org_id="o", project_id="p", local_path=restored,
    )
    assert ok is True
    ...
    assert (restored / ".git/objects/ab/cdef1234567890").read_bytes() == b"\x00\x01\x02BINARY"
...
async def test_download_rejects_mismatched_project_id(
    store, remote_root, project_workspace, tmp_path,
):
    ...
    # Upload as proj-A.
    await store.upload_project(
        org_id="o", project_id="proj-A", local_path=project_workspace,
    )
    ...
    restored = tmp_path / "restored"
    ok = await store.download_project(
        org_id="o", project_id="proj-B", local_path=restored,
    )
    assert ok is False
...
async def test_download_rejects_unknown_manifest_version(
    ...
    (key_dir / MANIFEST_FILENAME).write_text(json.dumps({
        "v": 99,  # from the future
        "org_id": "o", "project_id": "p", "file_count": 0,
        "total_bytes": 0, "executables": [],
    }))

    ok = await store.download_project(
        org_id="o", project_id="p", local_path=tmp_path / "restored",
    )
    assert ok is False
...
async def test_upload_skips_nonexistent_local_path(store, tmp_path):
    ...
    await store.upload_project(
        org_id="o", project_id="p", local_path=nope,
    )
    # No manifest written.
...
async def test_upload_swallows_fsspec_failure(
    monkeypatch, store, project_workspace,
):
