...
def test_create_client_injects_remote_control_into_extra_args(tmp_path: Path):
    ...
    with patch("core.client.ClaudeSDKClient", side_effect=_capture), \
        ...
        create_client(
            ...
            agent_type="coder",
            remote_control_session="AIFactory: 001-test-spec",
        )
    ...
    assert extra.get("remote-control") == "AIFactory: 001-test-spec", (
        f"Expected extra_args['remote-control']='AIFactory: 001-test-spec', "
        f"got {extra!r}"
    )
