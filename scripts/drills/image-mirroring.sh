...
#
# Mirrors an AIFactory image to a private registry preserving the
...
if [[ "$SHOW_HELP" == "1" ]]; then
    cat <<'EOF'
image-mirroring.sh — AIFactory v1.0 P7.6 image-mirroring drill
...
main() {
    echo "=== AIFactory image-mirroring drill ==="
    if [[ "$DRY_RUN" == "1" ]]; then
        echo "Mode: DRY-RUN"
    else
        echo "Mode: LIVE"
