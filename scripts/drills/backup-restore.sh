...
if [[ "$SHOW_HELP" == "1" ]]; then
    cat <<'EOF'
backup-restore.sh — AIFactory v1.0 P7.6 backup-restore drill
...
main() {
    echo "=== AIFactory backup-restore drill ==="
    if [[ "$DRY_RUN" == "1" ]]; then
    ...
    step "1. pg_dump source DB to /tmp/aifactory-drill-\$\$.dump"
    if [[ "$DRY_RUN" == "0" ]]; then
        pg_dump --format=custom \
            --file="/tmp/aifactory-drill-$$.dump" \
            "$DATABASE_URL"
    fi
