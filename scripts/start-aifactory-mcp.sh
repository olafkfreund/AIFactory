#!/usr/bin/env bash
#
# Spawn the AIFactory MCP server for Claude Code via stdio.
...
if [ ! -x "$ROOT/apps/backend/.venv/bin/python" ]; then
  for candidate in "${AIFACTORY_PROJECT_DIR:-}" "${CLAUDE_PROJECT_DIR:-}"; do
...
if [ ! -x "$PYTHON" ]; then
  cat >&2 <<EOF
aifactory MCP server cannot start: Python venv missing at
    $PYTHON

From the AIFactory repo root run:
...
exec "$PYTHON" -m mcp_server.aifactory_server "$@"
