...
#   - jq, curl
#   - Portal running at http://localhost:3101 (or pass --portal=URL)
#   - ~/.aifactory/.token exists (created by the portal at first boot)
#
# Flags:
#   --yolo         run uninterrupted (no Enter prompts between steps)
#   --no-reset     skip step 1 (don't touch the GitHub repo)
#   --portal=URL   override the portal base URL (default http://localhost:3101)
...
if [ "$NO_RESET" = "1" ]; then
  ...
  if ! gh repo view "$DEMO_REPO" >/dev/null 2>&1; then
    ...
    echo "      --description 'AIFactory demo project'"
...
REGISTER_RESPONSE=$(curl -sS -w '\n%{http_code}' -X POST "${PORTAL}/api/projects" \
  ...
  -d "{\"path\":\"${DEMO_REPO_LOCAL}\",\"name\":\"aifactory-demo\"}")
...
cat <<'EOF'
  ...
  In a SEPARATE terminal:
    ...
    cd /tmp/aifactory-demo
    claude

  Then ask Claude:

    "Refine the spec for the /healthz task. Edit .aifactory/specs/001-*/spec.md
     to specify the exact JSON response shape and add a unit-test acceptance
     criterion. Also tighten implementation_plan.json so subtask 1.1 reads
...
cat <<EOF
  ...
  Tail the agent logs in this terminal:
    ...
    curl -sN -H "Authorization: Bearer \$(cat ~/.aifactory/.token)" \\
