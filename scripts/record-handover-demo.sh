#!/usr/bin/env bash
# record-handover-demo.sh — terminal demo of the AIFactory handover workflow.
#
...
banner "AIFactory — terminal handover workflow"

cat <<EOF
${BOLD}The 60-second story:${RESET}
  A developer wants to ship a feature.  They use Claude Code to write
  the spec, then type ${BOLD}/handover${RESET} to send it to AIFactory's
  autonomous pipeline.  AIFactory plans → codes → QAs → opens a PR.
...
narrate "Standard clone — nothing AIFactory-specific."
...
conversation, calls ${BOLD}mcp__aifactory__task_create_and_run${RESET}, and
returns a portal URL.  AIFactory's autonomous pipeline takes it
from there.

EOF
sleep 3

# ── Step 4: the artifacts AIFactory produces ─────────────────────────────

banner "Step 4 — the spec AIFactory wrote"
...
banner "Done — that's the AIFactory handover workflow"

cat <<EOF
${BOLD}Try it yourself:${RESET}
  ${CYAN}https://olafkfreund.github.io/AIFactory/getting-started${RESET}
  ${CYAN}https://github.com/olafkfreund/AIFactory/blob/main/guides/HANDOVER_WORKFLOW.md${RESET}
