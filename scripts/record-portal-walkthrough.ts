 ...
 *   2. ~/.aifactory/.token present (set when the backend booted)
...
const PORTAL_URL = process.env.AIFACTORY_PORTAL ?? 'http://localhost:3100';
const PROJECT_NAME = process.env.AIFACTORY_PROJECT_NAME ?? 'aifactory-demo';
// UUID of the aifactory-demo project — discovered via the live portal's
...
const PROJECT_ID = process.env.AIFACTORY_PROJECT_ID ?? 'ac62db91-a89e-4238-85cb-3befdd2a7139';
...
> Built with AIFactory — autonomous spec-driven development.
...
async function main(): Promise<void> {
  ...
  const newCard = page
    ...
    .filter({hasText: /AIFactory|Append|README/i})
    .first();
  await newCard.click();
