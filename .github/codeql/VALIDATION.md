# Validating the custom sanitizer packs

`custom-queries/PathInjectionSanitized.ql` re-emits `py/path-injection` and
`custom-queries/CommandInjectionSanitized.ql` re-emits
`py/command-line-injection`, both with this repo's barriers registered, and
`codeql-config.yml` excludes the stock rules in their favour.
